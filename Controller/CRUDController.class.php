...
class CRUDController extends Controller {
	...
	//列出所有记录页面代码（片段）
	public function generateAllPageCode($theme){
		$templateFilePath = MODULE_PATH. "Template/View/".$theme."/allCode.html";
		$tableName = I('table');
		$moduleName = I('moduleName');
		$TableName = tableNameToModelName($tableName);
		$tableInfoArray = getTableInfoArray($tableName);
		$columnNameKey = getColumnNameKey();
		$commentNameKey = getCommentNameKey();
		$this->assign('tableName', $tableName);
		$this->assign('moduleName',$moduleName);
		$this->assign('TableName', $TableName);
		$this->assign('tableInfoArray', $tableInfoArray);
		$this->assign('columnNameKey', $columnNameKey);
		$this->assign('commentNameKey',$commentNameKey);
		$primaryKey;
		foreach($tableInfoArray as $t){
			if($t['column_key']=='PRI'){
				$primaryKey = $t[$columnNameKey];
			}
		}
	...
	//生成新建页面代码（片段）
	public function generateAddPage($theme){ 
		$templateFilePath = MODULE_PATH. "Template/View/".$theme."/addCode.html";
		$tableName = I('table'); 
		$TableName = tableNameToModelName($tableName);
		$tableInfoArray = getTableInfoArray($tableName);
		$columnNameKey = getColumnNameKey();
	...
	//编辑页面
	public function generateEditPage($theme){	
		$templateFilePath = MODULE_PATH. "Template/View/".$theme."/editCode.html";
		$tableName = I('table'); 
		$TableName = tableNameToModelName($tableName);
		$tableInfoArray = getTableInfoArray($tableName);
		$columnNameKey = getColumnNameKey();
	...
	public function creatAllFiles(){
		...
		$moduleName = I('moduleName');
		$baseController = I('base_controller');
		$theme = I('theme');
		...
		for($i = 0;$i < count($tableName); $i++){
			...
			$originalAllViewStr = $this->generateAllPageCode($theme);
			$allViewStr = $this->makeViewTemplate($theme."/index.html", $tableName[$i], $originalAllViewStr);
			$originalAddViewStr = $this->generateAddPage($theme);
			$addViewStr = $this->makeViewTemplate($theme."/add.html", $tableName[$i], $originalAddViewStr);
			$originalEditViewStr = $this->generateEditPage($theme);
			$editViewStr = $this->makeViewTemplate($theme."/edit.html", $tableName[$i], $originalEditViewStr);
			...
			file_put_contents($viewPath."index.html", $allViewStr);
